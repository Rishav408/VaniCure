...
export interface ScreeningRecord {
  ...
  overallResult: string;  // "High Risk TB" | "Asthma Pattern" | "Normal" etc.
...
const SEED_DATA: ScreeningRecord[] = [
  {
    ...
    location: "Village A", date: "2026-03-21", time: "10:15 AM",
  ...
  },
  {
    ...
    location: "Village B", date: "2026-03-21", time: "09:30 AM",
    overallResult: "Asthma Pattern", riskLevel: "warning", confidence: 92.1,
    models: [
  ...
  },
  {
    ...
    location: "Village A", date: "2026-03-20", time: "08:45 AM",
    overallResult: "Normal", riskLevel: "safe", confidence: 99.2,
    models: [
  ...
  },
  {
    ...
    location: "Village C", date: "2026-03-20", time: "07:20 AM",
    overallResult: "Normal", riskLevel: "safe", confidence: 98.5,
    models: [
      { model: "PANNs-CNN14", tb_risk: 0.02, asthma_risk: 0.03, normal: 0.95 },
      { model: "YAMNet", tb_risk: 0.01, asthma_risk: 0.02, normal: 0.97 },
    ],
    audioDurationSec: 3.5, synced: true,
  },
  {
    id: "VC-888", patientName: "Kishan Lal", age: 61, gender: "M",
    location: "Village B", date: "2026-03-19", time: "04:40 PM",
    overallResult: "COPD Risk", riskLevel: "warning", confidence: 76.8,
    models: [
      { model: "PANNs-CNN14", tb_risk: 0.15, asthma_risk: 0.62, normal: 0.23 },
      { model: "YAMNet", tb_risk: 0.12, asthma_risk: 0.58, normal: 0.30 },
    ],
    audioDurationSec: 6.0, synced: true,
  },
  {
    id: "VC-887", patientName: "Pooja Singh", age: 19, gender: "F",
    location: "Village A", date: "2026-03-19", time: "02:10 PM",
    overallResult: "Normal", riskLevel: "safe", confidence: 99.9,
    models: [
      { model: "PANNs-CNN14", tb_risk: 0.00, asthma_risk: 0.01, normal: 0.99 },
      { model: "YAMNet", tb_risk: 0.01, asthma_risk: 0.00, normal: 0.99 },
    ],
    audioDurationSec: 4.0, synced: true,
  },
  {
    id: "VC-886", patientName: "Raju Bhai", age: 52, gender: "M",
    location: "Village B", date: "2026-03-19", time: "11:05 AM",
    overallResult: "High Risk TB", riskLevel: "critical", confidence: 89.1,
    models: [
      { model: "PANNs-CNN14", tb_risk: 0.91, asthma_risk: 0.04, normal: 0.05 },
      { model: "YAMNet", tb_risk: 0.87, asthma_risk: 0.06, normal: 0.07 },
    ],
...
/** Detect outbreak anomalies — locations with ≥2 high-risk cases in recent data. */
export function getOutbreakAlerts() {
  ...
  for (const [location, recs] of Object.entries(locationMap)) {
    ...
    if (critical.length >= 2) {
      alerts.push({
        id: id++, region: location, type: "TB Cluster",
        cases: critical.length, totalScreenings: recs.length,
        status: "Critical", latestTime: critical[0].time,
      });
    }
    if (warning.length >= 2) {
      alerts.push({
        id: id++, region: location, type: "Respiratory Spike",
        cases: warning.length, totalScreenings: recs.length,
        status: "Warning", latestTime: warning[0].time,
      });
    }
    if (critical.length === 1) {
      alerts.push({
        id: id++, region: location, type: "TB Case Detected",
        cases: 1, totalScreenings: recs.length,
        status: "Monitoring", latestTime: critical[0].time,
      });
    }
  }
...
export function exportCSV() {
  const records = getScreenings();
  const headers = [
    "Scan ID", "Patient Name", "Age", "Gender", "Location",
  ...
  const rows = records.map(r => {
    ...
    return [
      r.id, r.patientName, r.age, r.gender, r.location,
